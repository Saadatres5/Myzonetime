...
import { Menu, Clock, Globe2, ArrowRightLeft, Calendar, Moon, Code, Timer, Sun, Droplet } from 'lucide-react';
...
export default function Header({ settings, updateSetting }) {
  ...
  const selectTheme = (theme) => updateSetting('theme', theme);
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8" aria-label="Main navigation">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="rounded-full px-4 py-2 border-border text-muted-foreground hover:text-foreground">
                {themeOptions.find((option) => option.value === settings.theme)?.label || 'Theme'}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="min-w-[12rem]">
              <DropdownMenuLabel>Theme</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuRadioGroup value={settings.theme} onValueChange={selectTheme}>
